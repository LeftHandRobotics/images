...
// Comments or suggestions to work@tbyrne.org
// 
...
var multi_exporter = {
    ...
    PREFS_LAYER_NAME: "MultiExporter Props",

    DEFAULT_PATTERN: "<ArtboardName>_<LayerName>.<Ext>",
    ...
    formatInfo: [   {name:"PNG 8", ext:'png', copyBehaviour:false, getOptions:getPng8Options, saveFile:savePng8, activeControls:["scalingInput","transCheckBox","trimEdgesCheckBox","innerPaddingCheckBox"]},
                    {name:"PNG 24", ext:'png', copyBehaviour:false, getOptions:getPng24Options, saveFile:savePng24, activeControls:["scalingInput","transCheckBox","trimEdgesCheckBox","innerPaddingCheckBox"]},
                    {name:"PDF", ext:'pdf', copyBehaviour:false, getOptions:getPdfOptions, saveFile:savePdf, activeControls:["trimEdgesCheckBox"]},
                    {name:"JPG", ext:'jpg', copyBehaviour:false, getOptions:getJpgOptions, saveFile:saveJpg, activeControls:["scalingInput","trimEdgesCheckBox","innerPaddingCheckBox"]},
                    {name:"EPS", ext:'eps', copyBehaviour:true, getOptions:getEpsOptions, saveFile:saveEps, activeControls:["embedImageCheckBox","embedFontCheckBox","trimEdgesCheckBox"]},
                    {name:"SVG 1.1", ext:'svg', copyBehaviour:true, getOptions:getSvgOptions, saveFile:saveSvg, activeControls:["embedImageCheckBox","trimEdgesCheckBox"]},
                    {name:"FXG 1.0", ext:'fxg', copyBehaviour:true, getOptions:getFxg1Options, saveFile:saveFxg, activeControls:["trimEdgesCheckBox"]},
                    {name:"FXG 2.0", ext:'fxg', copyBehaviour:true, getOptions:getFxg2Options, saveFile:saveFxg, activeControls:["trimEdgesCheckBox"]}],
    ...
    load_prefs: function() {
        ...
        try {
            ...
            try {
                this.multiExporterPrefs = docRef.layers.getByName( this.OLD_PREFS_LAYER_NAME );
                ...
                var exporter_info_xml = this.multiExporterPrefs.textFrames.add();
                
                var saved_data = new XML( '<prefs></prefs>' );
                saved_data.appendChild( new XML('<pattern>'+this.xmlEncode(this.DEFAULT_PATTERN)+'</pattern>') );
                saved_data.appendChild( new XML('<base_path>~/Desktop</base_path>') );
                saved_data.appendChild( new XML('<scaling>100%</scaling>') );
                saved_data.appendChild( new XML('<transparency>true</transparency>') );
                saved_data.appendChild( new XML('<embedImage>true</embedImage>') );
                saved_data.appendChild( new XML('<embedFont>true</embedFont>') );
                saved_data.appendChild( new XML('<trimEdges>true</trimEdges>') );
                saved_data.appendChild( new XML('<innerPadding>false</innerPadding>') );
                saved_data.appendChild( new XML('<format>PNG 24</format>') );
                saved_data.appendChild( new XML('<artboards>all</artboards>') );
                saved_data.appendChild( new XML('<layers>all</layers>') );
                saved_data.appendChild( new XML('<exportArtboards>false</exportArtboards>') );
                saved_data.appendChild( new XML('<ignoreWarnings>false</ignoreWarnings>') );
                exporter_info_xml.contents = saved_data.toXMLString();  
        ...
        if ( this.multiExporterPrefs.textFrames.length != 1 ) {
            ...
            try {
                ...
                this.updatePrefNode(this.prefs_xml);
                this.multiExporterPrefs.textFrames[0].contents = this.prefs_xml.toXMLString();
                
                this.pattern        = this.prefs_xml.pattern || this.DEFAULT_PATTERN;
                this.base_path      = this.prefs_xml.base_path;
                this.scaling        = this.prefs_xml.scaling;
                this.transparency   = this.prefs_xml.transparency == "true" ? true : false;
                this.embedImage     = this.prefs_xml.embedImage == "true" ? true : false;
                this.ignoreWarnings = this.prefs_xml.ignoreWarnings == "true" ? true : false;
                this.embedFont      = this.prefs_xml.embedFont == "true" ? true : false;
                this.trimEdges      = this.prefs_xml.trimEdges == "true" ? true : false;
                this.innerPadding   = this.prefs_xml.innerPadding == "true" ? true : false;
                this.format         = this.prefs_xml.format;
                this.artboards      = this.prefs_xml.artboards.toString();
                this.layers         = this.prefs_xml.layers.toString();
                this.whole_artboard_mode = this.prefs_xml.exportArtboards == "true" ? true : false;
    ...
    patternChanged:function(){
        if(!this.export_layers || !this.currentFormatInfo){
            this.exampleSt.text =  "";
            return;
        }
        var example;
        if(this.export_layers.length && this.export_artboards.length){
            var artboardI = this.export_artboards[0];
            var layerI = this.export_layers[0];
            this.exampleSt.text =  this.makeFileName(this.patternEt.text, this.currentFormatInfo.ext, artboardI+1, docRef.artboards[artboardI].name, layerI+1, docRef.layers[layerI].name);
        }else if(this.export_artboards.length){
            var artboardI = this.export_artboards[0];
            this.exampleSt.text =  this.makeFileName(this.patternEt.text, this.currentFormatInfo.ext, artboardI+1, docRef.artboards[artboardI].name, "", "");
        }else{
            this.exampleSt.text =  "";
        }
    },
    ...
    addToken:function(){
        if(this.tokenList.selection>0){
            var selected = Number(this.tokenList.selection);
            var token = this.TOKENS[selected];
            this.patternEt.text += token;
            this.patternChanged();
        }
        this.tokenList.selection = 0;
    },
    ...
    show_dialog: function() {
        ...
        var patternSt = row.add('statictext', undefined, 'Filename Pattern:'); 
        patternSt.size = [100,20]

        this.patternEt = row.add('edittext', undefined, this.pattern); 
        this.patternEt.size = [ 300,20 ];
        this.patternEt.addEventListener("keyup", function() {
            multi_exporter.patternChanged();
        });
        this.patternEt.onChange = function() {
            multi_exporter.patternChanged();
        };

        this.tokenList = row.add('dropdownlist', undefined, this.TOKENS);
        this.tokenList.onChange = function() {
            multi_exporter.addToken();
        };
        this.tokenList.selection = 0;
        ...
        var patternSt = row.add('statictext', undefined, 'Example Filename:'); 
        patternSt.size = [105,20]

        this.exampleSt = row.add('statictext', undefined, ''); 
        this.exampleSt.size = [ 300,20 ];
    ...
    saveOptions:function(){
        this.pattern     = this.patternEt.text;
        ...
        this.prefs_xml.base_path    = this.base_path;
        this.prefs_xml.scaling    = this.scaling;
        this.prefs_xml.pattern     = this.pattern;
        this.prefs_xml.transparency = this.transparency;
        this.prefs_xml.embedImage = this.embedImage;
        this.prefs_xml.ignoreWarnings = this.ignoreWarnings;
        this.prefs_xml.embedFont = this.embedFont;
        this.prefs_xml.trimEdges = this.trimEdges;
        this.prefs_xml.innerPadding = this.innerPadding;
        this.prefs_xml.format      = this.format;
        this.prefs_xml.artboards  = this.artboards;
        this.prefs_xml.layers  = this.layers;
        this.prefs_xml.exportArtboards  = this.whole_artboard_mode;
    ...
    run_export: function() {
        ...
        for (var i = 0; i < this.export_artboards.length; i++ ) {
            ...
            if ( this.whole_artboard_mode) {
                ...
                try{
                    var base_filename = this.base_path + "/" + this.makeFileName(this.pattern, formatInfo.ext, i+1, artboardName, "", "");//this.base_path + "/" + this.prefix + artboardName + this.suffix;
                    if(copyBehaviour){
                ...
                for ( var j=0; j < this.export_layers.length; j++ ) {
                    ...
                    try{
                        ...
                        if (layerRect[0]<layerRect[2] && layerRect[1]>layerRect[3]) {
                            ...
                            var base_filename = this.base_path + "/" + this.makeFileName(this.pattern, formatInfo.ext, i+1, artboardName, j+1, lyr_name);
        ...
        for ( var j=docRef.layers.length-1; j >=0; j-- ) {
            ...
            if (layerCheck(layer)) {
                ...
                if(layerBounds && this.intersects(rect, layerBounds)){
                    this.copy_layer(layer, copyDoc.layers.add(), offset, w, h, this.innerPadding);
                    ++count;
                }
            }else if(layerDepths){
                layerDepths[j] = count;
